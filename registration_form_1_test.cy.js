...
beforeEach(() => {
 ...
 // 13.06.2023
describe('This is first test suite, Kadi Kangur', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Kadi')
        cy.get('#lastName').type('Kangur')
        cy.get('input[name="password"]').type('MyNewPass123')
        cy.get('[name="confirm"]').type('MyNewPass123')
    ...
    it('User can use only same both first and validation passwords', () => {
        ...
        cy.get('input[name="password"]').type('MyNewPass123')
        cy.get('[name="confirm"]').type('MyNew')
